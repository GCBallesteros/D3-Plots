...
(function() {
  ...
  heatmap = (function() {
    ...
    heatmap.prototype.plot = function() {
      var color_brewer, cursor, data, extX, extY, h, height_tile, height_tile_px, maxZ, minZ, svg, tiles, w, width_tile, width_tile_px, xPoints, xScale, yPoints, yScale, zHeight, zScale;
      ...
      tiles = svg.append("g").attr("class", "tiling").on("mousemove", function() {
        var cursor, mousePos;
        mousePos = d3.mouse(this);
        cursor = d3.select("#cursor").style("display", "inline");
        cursor.select(".xLine").style("display", "inline").attr("x1", mousePos[0]).attr("y1", mousePos[1]).attr("y2", mousePos[1]);
        cursor.select(".yLine").style("display", "inline").attr("y1", mousePos[1]).attr("x1", mousePos[0]).attr("x2", mousePos[0]);
        return cursor.select(".xPos").attr("x", mousePos[0]).text(xScale.invert(mousePos[0]));
      });
      tiles.selectAll(".tile").data(heatmap.prototype.flatten(zHeight)).enter().append("rect").attr("class", "tile").attr("x", function(d, i) {
      ...
      cursor = svg.append("g").attr("id", "cursor").style("display", "none");
      cursor.append("line").attr("class", "xLine").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", 1).style("stroke", "black").style("stroke-dasharray", "3, 3").style("stroke-width", "1px");
      cursor.append("text").attr("class", "xPos").attr("x", 0).attr("y", h + 12).attr("text-anchor", "middle").attr("font-family", "sans-serif").attr("font-size", "11px").attr("font-weight", "bold").attr("fill", "black");
      cursor.append("line").attr("class", "yLine").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", h).style("stroke", "black").style("stroke-dasharray", "3, 3").style("stroke-width", "1px");
      svg.append;
      return void 0;
    };
