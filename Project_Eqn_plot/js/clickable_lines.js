...
function chart() {
    ...
    function plot() {
        ...
        var clickable_lines = svg.append("g")
                                 .attr("class", "clickLines");
        ...
        for (var i = 0, len = datase.length; i < len; i++) {
            ...
            // Creat clickable lines
            //
            // 1) Create the line we see
            var line_group = clickable_lines.append("g")
            ...
            // 2) Create the line that gives a wider area for hoovering
            //    and the one that actually triggers the events
            line_group.append("path")
               .datum(curr_line.data)
               .attr("class", "hoover_line")
               .attr("d", line)
               .attr("opacity", "0.0")
               .style("stroke-width", "25px")
               .style("stroke", "black")
               .on("click", plot.highlight_line)
        }
    };
    ...
    plot.highlight_line = function () {
        d3.select(this.parentNode.parentNode).selectAll(".clickLine")
          .selectAll(".line")
          .style("opacity", 0.4);
    ...
    plot.generate_data = function () {
        ...
        return [{label: "Cos", data: d3.transpose([xdata, ydata])},
